...
abstract class CstObjBase(V, LEAF, alias R, int N)
  if(_esdl__ArrOrder!(V, N) == 0): CstObjDomain!(LEAF, R), CstObjPrim
{
...
template CstObj(T, int I, int N=0)
  ...
  alias CstObj = CstObj!(typeof(T.tupleof[I]), typeof(T.tupleof[I]),
			 getRandAttr!(T, I), N);
}

class CstObj(V, LEAF, alias R, int N) if(N == 0 && _esdl__ArrOrder!(V, N) == 0):
  CstObjBase!(V, LEAF, R, N)
    {
      import std.traits;
      import std.range;
      import esdl.data.bvec;
	...
	assert(false);
      ...
      // RV
      CstObjExpr unroll(CstIteratorBase iter, uint n) {
  ...
  CstObjBase!(V, LEAF, R, N)
    {
      ...
      alias RV = typeof(this);
      alias P = CstObjArr!(V, LEAF, R, N-1);
      P _parent;
...
abstract class CstObjArrBase(V, LEAF, alias R, int N)
  if(_esdl__ArrOrder!(V, N) != 0): CstVecPrim
{
  ...
  static if (_esdl__ArrOrder!(V, N+1) == 0) {
    alias EV = CstObj!(V, LEAF, R, N+1);
  }
  else {
    alias EV = CstObjArr!(V, LEAF, R, N+1);
  }
